...
export default function BaseProjectCard({ project }) {
  const { status, title, description, date, participants, maxParticipants } = project;
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border bg-white shadow-sm transition-transform hover:-translate-y-1">
      ...
      <div className="border-t bg-gray-50 p-4">
        <button className="w-full rounded-lg bg-[#6080A4] py-2.5 text-sm font-semibold text-white transition hover:bg-[#526d8c]">
          Join Now
        </button>
      </div>
    </div>
